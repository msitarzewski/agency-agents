# Multi-Agent Workflow: Startup MVP with Memory

> The same startup MVP workflow from [workflow-startup-mvp.md](workflow-startup-mvp.md), but with Novyx memory handling state between agents. No more copy-paste handoffs.
...
With `novyx-mcp` installed, agents store their deliverables in memory and retrieve what they need automatically. Handoffs become:
...
No API key needed. Memories are stored locally in `~/.novyx/local.db`.
...
Each agent has a Memory Integration section in their prompt (see [integrations/novyx-memory/README.md](../integrations/novyx-memory/README.md) for how to add it).
...
| **Setup required** | None | `pip install novyx-mcp` |
...
- With `novyx-mcp` in local mode, this entire workflow works offline with zero configuration
