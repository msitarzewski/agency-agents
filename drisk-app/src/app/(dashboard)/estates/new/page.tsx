...
import { getClientsForSelect } from "@/app/actions/estate";
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-3xl mx-auto p-4 md:p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Add Estate</h1>
          <p className="text-slate-500 dark:text-slate-400">Create a new estate to group related client sites.</p>
        </div>
      </div>

      <div className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6">
        <EstateForm clients={clients} />
      </div>
    </div>
  );
}
