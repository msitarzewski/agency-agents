...
#   codex        -- Copy Codex plugin to ~/.codex/plugins/agency-agents/
...
tool_label() {
  case "$1" in
    ...
    codex)       printf "%-14s  %s" "Codex"        "(~/.codex/plugins)"      ;;
...
install_codex() {
  ...
  local dest="${codex_home}/plugins/agency-agents"

  [[ -d "$src" ]] || { err "integrations/codex missing. Run convert.sh first."; return 1; }
  [[ -d "$src/.codex-plugin" ]] || { err "integrations/codex/.codex-plugin missing. Run convert.sh first."; return 1; }

  mkdir -p "$dest"
  rm -rf "$dest/.codex-plugin" "$dest/assets" "$dest/skills" "$dest/references"

  cp -R "$src/.codex-plugin" "$dest/.codex-plugin"
  cp -R "$src/assets" "$dest/assets"
  cp -R "$src/skills" "$dest/skills"
  cp -R "$src/references" "$dest/references"

  ok "Codex: plugin installed -> $dest"
  warn "Codex: restart Codex or reload plugins if the plugin catalog is already open."
}
