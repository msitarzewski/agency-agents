...
convert_zcode() {
  ...
  # Stage one dir per skill (install.sh copies into ~/.zcode/skills/<name>/).
  outdir="$OUT_DIR/zcode/$slug"
  outfile="$outdir/SKILL.md"
  mkdir -p "$outdir"

  # ZCode skill format: a directory named for the skill containing a SKILL.md
  # with name + description frontmatter and the persona as the instruction body.
  # Installs into ~/.zcode/skills/. Same format as Osaurus/Antigravity.
  cat > "$outfile" <<HEREDOC
---
name: ${slug}
description: ${description}
...
main() {
  ...
  local valid_tools=("antigravity" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "kimi" "codex" "osaurus" "hermes" "zcode" "all")
  local valid=false
  ...
  local tools_to_run=()
  if [[ "$tool" == "all" ]]; then
    tools_to_run=("antigravity" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "kimi" "codex" "osaurus" "hermes" "zcode")
  else
    tools_to_run=("$tool")
  fi
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    ...
    local parallel_tools=(antigravity gemini-cli opencode cursor openclaw qwen codex osaurus hermes zcode)
