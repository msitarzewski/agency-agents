...
#   codex        — Codex multi-agent roles (.codex/config.toml + agents/*.toml)
...
# Strip control characters that TOML multiline strings cannot contain.
sanitize_toml_multiline() {
...
convert_antigravity() {
  ...
  body="$(sanitize_toml_multiline "$(get_body "$file")")"
...
convert_codex() {
  local file="$1"
  local name description slug outfile body escaped_description

  name="$(get_field "name" "$file")"
  description="$(get_field "description" "$file")"
  slug="$(slugify "$name")"
  body="$(sanitize_toml_multiline "$(get_body "$file")")"
  escaped_description="$(toml_escape_basic_string "$description")"

  outfile="$OUT_DIR/codex/agents/${slug}.toml"
  mkdir -p "$(dirname "$outfile")"

  # Codex role config: keep overrides minimal so each role inherits the user's
  # parent session model/sandbox choices and only injects developer instructions.
  cat > "$outfile" <<HEREDOC
...
WINDSURF_TMP="$(mktemp)"
CODEX_TMP="$(mktemp)"
trap 'rm -f "$AIDER_TMP" "$WINDSURF_TMP" "$CODEX_TMP"' EXIT
...
main() {
  ...
  local valid_tools=("antigravity" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "codex" "all")
  local valid=false
  ...
  local tools_to_run=()
  if [[ "$tool" == "all" ]]; then
    tools_to_run=("antigravity" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "codex")
  else
    tools_to_run=("$tool")
  fi
...
HEREDOC
  ...
  if [[ "$tool" == "all" || "$tool" == "codex" ]]; then
    ...
    cp "$CODEX_TMP" "$OUT_DIR/codex/config.toml"
    info "Wrote integrations/codex/config.toml"
  fi
