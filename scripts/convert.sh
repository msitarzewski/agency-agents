...
#   codex        — Codex skill folders (~/.codex/skills/<skill>/SKILL.md)
...
build_codex_slug() {
  local file="$1"
  local relative parent stem prefix
  ...
  relative="$(dirname "$relative")/$stem"
  relative="${relative#./}"
  printf 'agency-%s\n' "$(slugify "${relative//\//-}")"
}
...
main() {
  ...
  local valid_tools=("antigravity" "gemini-cli" "opencode" "codex" "cursor" "aider" "windsurf" "openclaw" "qwen" "kimi" "all")
  local valid=false
  ...
  local tools_to_run=()
  if [[ "$tool" == "all" ]]; then
    tools_to_run=("antigravity" "gemini-cli" "opencode" "codex" "cursor" "aider" "windsurf" "openclaw" "qwen" "kimi")
  else
    tools_to_run=("$tool")
  fi
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    ...
    local parallel_tools=(antigravity gemini-cli opencode codex cursor openclaw qwen)
