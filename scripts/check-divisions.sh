...
# runbooks (no agent frontmatter), not agents. plugins/ is the generated plugin
# marketplace tree (plugins/<division>/commands/*.md), not a source-agent
# category. None of these are divisions — they must never be scanned as
# source-agent categories.
NON_DIVISION_DIRS=(examples scripts integrations strategy plugins)
