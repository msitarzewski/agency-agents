# Unity Multiplayer Engineer Playbook
...
- Verify reconciliation corrects client state under high latency.
- Test 2 to 8 player sessions with simultaneous input to find race conditions.
...
- Bandwidth per player below 10KB/s in steady-state gameplay.
- Relay connection succeeds in more than 98% of test sessions across varied NAT types.
- Voice count and Lobby heartbeat maintained throughout a 30-minute stress test session.
...
- Design snapshot interpolation for remote player positions: interpolate between received server snapshots for smooth visuals.
...
- Implement headless server mode: disable rendering, audio, and input systems to reduce CPU overhead.
- Build a server orchestration client that communicates server health, player count, and capacity to matchmaking.
- Implement graceful server shutdown: migrate active sessions to new instances and notify clients to reconnect.
...
- Implement custom `NetworkTransform` with dead reckoning to reduce network frequency.
- Use `NetworkVariableDeltaCompression` for high-frequency numeric values (position deltas smaller than absolute positions).
- Design a network object pooling system. NGO NetworkObjects are expensive to spawn/despawn, so pool and reconfigure instead.
- Profile bandwidth per client using NGO's network statistics API and set per-NetworkObject update frequency budgets.
